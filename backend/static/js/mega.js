...
// Проверяем, зарегистрирован ли пользователь через Django сессию
let isRegistered = false;
let userData = {};

// Проверяем валидность сессии через Django API
(async () => {
    try {
        const response = await apiClient.validateSession();
        if (response.success && response.valid && response.is_authenticated) {
            // Пользователь авторизован через Django сессию
            isRegistered = true;
            userData = {
                id: response.user.id,
                username: response.user.username,
                email: response.user.email,
                registrationDate: response.user.date_joined
            };
    ...
    // Показываем кнопку "Начать играть" если пользователь авторизован
    if (isRegistered) {
        showPlayButton();
        updateLoginButton();
    }
...
// Загружаем настройки из API после проверки авторизации
setTimeout(async () => {
    if (isRegistered) {
        try {
            ...
            if (response.success && response.settings) {
                ...
                if (settings.email_notifications !== undefined) {
                    settingsManager.settings.notifications.email = settings.email_notifications;
                }
                if (settings.game_notifications !== undefined) {
                    settingsManager.settings.notifications.games = settings.game_notifications;
                }
                if (settings.news_notifications !== undefined) {
                    settingsManager.settings.notifications.news = settings.news_notifications;
                }
...
loginFormElement.addEventListener('submit', async function(e) {
    ...
    try {
        ...
        if (response.success) {
            // Сохраняем данные пользователя (Django сессия уже создана)
            userData = {
                id: response.user.id,
                username: response.user.username,
                email: response.user.email,
            ...
            showPlayButton();
            updateLoginButton();
            showSuccessMessage(`${languageManager.getText('welcomeBack')}, ${response.user.username}!`);
        }
    } catch (error) {
        const errorMessage = error.message || languageManager.getText('wrongPassword');
...
regForm.addEventListener('submit', async function(e) {
    ...
    try {
        ...
        const errorMessage = error.message || languageManager.getText('userExists');
        showError(errorMessage);
    }
});
...
document.querySelectorAll('.language-btn').forEach(btn => {
    btn.addEventListener('click', async function() {
        ...
        // Сохраняем в Django API, если пользователь авторизован
        if (isRegistered) {
            try {
...
document.getElementById('gameNotifications').addEventListener('change', async function() {
    ...
    settingsManager.saveSettings();
        ...
        if (isRegistered) {
            try {
                await apiClient.updateSettings({
                    game_notifications: this.checked
                });
        } catch (error) {
            console.error('Ошибка обновления настроек:', error);
        }
...
document.getElementById('newsNotifications').addEventListener('change', async function() {
    ...
    settingsManager.saveSettings();
        ...
        if (isRegistered) {
            try {
                await apiClient.updateSettings({
                    news_notifications: this.checked
                });
        } catch (error) {
            console.error('Ошибка обновления настроек:', error);
        }
...
async function confirmLogout() {
    ...
    const modal = document.querySelector('.logout-confirmation');
...
async function confirmDeleteAccount() {
    try {
        // Удаление аккаунта через Django API
...
async function loginUser(user, rememberMe) {
    // Функция теперь не используется, так как вход обрабатывается в обработчике формы
    // Оставлена для совместимости
    try {
        const response = await apiClient.login(user.email, user.password, rememberMe);
        if (response.success) {
            userData = {
                id: response.user.id,
                username: response.user.username,
        ...
        showError(error.message || languageManager.getText('wrongPassword'));
    }
}
...
async function updateProfileData() {
    ...
    try {
        // Получаем профиль через Django API
        const response = await apiClient.getProfile();
        ...
        if (response.success) {
            ...
            // Статистика
            daysRegistered.textContent = profile.days_registered || 0;
            gamesPlayed.textContent = profile.games_played || 0;
            
            // Последний вход
            if (profile.last_login_time) {
                const lastLoginDate = new Date(profile.last_login_time);
            ...
            if (settings) {
                ...
                if (settings.game_notifications !== undefined) {
                    document.getElementById('gameNotifications').checked = settings.game_notifications;
                    settingsManager.settings.notifications.games = settings.game_notifications;
                }
                if (settings.news_notifications !== undefined) {
                    document.getElementById('newsNotifications').checked = settings.news_notifications;
                    settingsManager.settings.notifications.news = settings.news_notifications;
                }
        ...
        profileUsername.textContent = userData.username || 'Пользователь';
        profileEmail.textContent = userData.email || '';
    }
}
...
async function showChangeUsernameDialog() {
    ...
    if (newUsername && newUsername.trim() !== '' && newUsername !== userData.username) {
        if (validateUsername(newUsername.trim())) {
            try {
                ...
                const response = await apiClient.changeUsername(newUsername.trim());
                if (response.success) {
                ...
                showError(error.message || 'Ошибка изменения имени пользователя');
            }
        }
    }
}
...
async function showChangePasswordDialog() {
    ...
    if (validatePassword(newPassword)) {
        try {
            // Обновляем пароль через Django API
            ...
            showError(error.message || languageManager.getText('wrongCurrentPassword'));
        }
    }
}
...
async function showGameSelection() {
    ...
    // Загружаем игры из API
    try {
        const response = await apiClient.getGames();
        if (response.success && response.games && response.games.length > 0) {
            const gamesGrid = gameModal.querySelector('#gamesGrid');
            gamesGrid.innerHTML = '';
            
            response.games.forEach(game => {
                const gameCard = document.createElement('div');
                gameCard.className = 'game-card-modal';
                gameCard.onclick = () => startGame(game.game_type);
                gameCard.innerHTML = `
            ...
            // Fallback на старые игры, если API не вернул игры
            const gamesGrid = gameModal.querySelector('#gamesGrid');
            gamesGrid.innerHTML = `
...
window.startGame = async function(gameType) {
    ...
    // Запускаем сессию игры через Django API (если пользователь авторизован)
    if (isRegistered) {
        try {
            await apiClient.startGameSession(gameType);
        } catch (error) {
            console.error('Ошибка запуска игровой сессии:', error);
        }
    }
